import { jsxs, jsx } from 'react/jsx-runtime';
import { useState, useRef, useEffect } from 'react';
import { s as supabase } from './supabase_2zlJawaa.mjs';

function MessageThread({
  quoteId,
  initialMessages,
  userId,
  userName,
  currentLang = "zh"
}) {
  const [messages, setMessages] = useState(initialMessages || []);
  const [newMessage, setNewMessage] = useState("");
  const [sending, setSending] = useState(false);
  const [error, setError] = useState("");
  const messagesEndRef = useRef(null);
  const texts = {
    zh: {
      noMessagesPrompt: "还没有消息。如有疑问，请随时留言。",
      customerService: "客服",
      messagePlaceholder: "输入您的消息...",
      sendButton: "发送",
      sending: "发送中...",
      responsePrompt: "发送消息后我们的专家会尽快回复您",
      sendFailedPrefix: "发送失败: ",
      unknownError: "未知错误"
    },
    en: {
      noMessagesPrompt: "No messages yet. Feel free to leave a message if you have any questions.",
      customerService: "Support",
      messagePlaceholder: "Type your message...",
      sendButton: "Send",
      sending: "Sending...",
      responsePrompt: "Our experts will respond to your message as soon as possible",
      sendFailedPrefix: "Send failed: ",
      unknownError: "Unknown error"
    }
  };
  const t = texts[currentLang];
  const scrollToBottom = () => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  };
  useEffect(() => {
    scrollToBottom();
  }, [messages]);
  useEffect(() => {
    console.log("🔄 订阅报价消息 (V1.1):", quoteId);
    const channel = supabase.channel(`quote-messages-${quoteId}`).on("postgres_changes", {
      event: "INSERT",
      schema: "public",
      table: "quote_messages",
      filter: `quote_id=eq.${quoteId}`
    }, (payload) => {
      console.log("✅ 收到新消息 (V1.1):", payload.new);
      const newMsg = payload.new;
      setMessages((currentMessages) => {
        if (currentMessages.find((msg) => msg.id === newMsg.id)) {
          return currentMessages;
        }
        return [...currentMessages, newMsg];
      });
    }).subscribe();
    return () => {
      console.log("🛑 取消消息订阅");
      supabase.removeChannel(channel);
    };
  }, [quoteId]);
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (newMessage.trim() === "") return;
    if (sending) return;
    setSending(true);
    setError("");
    try {
      console.log("📤 发送消息 (V1.1):", newMessage);
      const { error: insertError } = await supabase.from("quote_messages").insert({
        quote_id: quoteId,
        sender_id: userId,
        message_content: newMessage.trim()
      });
      if (insertError) {
        throw insertError;
      }
      setNewMessage("");
      console.log("✅ 消息发送成功 (V1.1)");
    } catch (error2) {
      console.error("❌ 发送消息失败:", error2);
      setError(t.sendFailedPrefix + (error2.message || t.unknownError));
    } finally {
      setSending(false);
    }
  };
  const formatTimestamp = (timestamp) => {
    const date = new Date(timestamp);
    const now = /* @__PURE__ */ new Date();
    const diffInHours = (now.getTime() - date.getTime()) / (1e3 * 60 * 60);
    const locale = currentLang === "zh" ? "zh-CN" : "en-US";
    if (diffInHours < 24) {
      return date.toLocaleTimeString(locale, {
        hour: "2-digit",
        minute: "2-digit"
      });
    } else {
      return date.toLocaleDateString(locale, {
        month: "short",
        day: "numeric",
        hour: "2-digit",
        minute: "2-digit"
      });
    }
  };
  const isUserMessage = (message) => {
    return message.sender_id === userId;
  };
  return /* @__PURE__ */ jsxs("div", { className: "bg-white rounded-lg shadow-md overflow-hidden", children: [
    /* @__PURE__ */ jsxs("div", { className: "h-96 overflow-y-auto p-4 space-y-4 bg-gray-50", children: [
      messages.length === 0 ? /* @__PURE__ */ jsx("div", { className: "text-center text-gray-500 py-8", children: /* @__PURE__ */ jsx("p", { children: t.noMessagesPrompt }) }) : messages.map((message) => /* @__PURE__ */ jsx(
        "div",
        {
          className: `flex ${isUserMessage(message) ? "justify-end" : "justify-start"}`,
          children: /* @__PURE__ */ jsxs(
            "div",
            {
              className: `max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${isUserMessage(message) ? "bg-purple-600 text-white" : "bg-white text-gray-800 border border-gray-200"}`,
              children: [
                /* @__PURE__ */ jsx("div", { className: "text-sm", children: message.message_content }),
                /* @__PURE__ */ jsxs(
                  "div",
                  {
                    className: `text-xs mt-1 ${isUserMessage(message) ? "text-purple-200" : "text-gray-500"}`,
                    children: [
                      !isUserMessage(message) && /* @__PURE__ */ jsxs("span", { className: "font-medium", children: [
                        t.customerService,
                        " · "
                      ] }),
                      formatTimestamp(message.created_at)
                    ]
                  }
                )
              ]
            }
          )
        },
        message.id
      )),
      /* @__PURE__ */ jsx("div", { ref: messagesEndRef })
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "border-t bg-white p-4", children: [
      error && /* @__PURE__ */ jsx("div", { className: "mb-3 p-2 bg-red-50 border border-red-200 rounded text-red-700 text-sm", children: error }),
      /* @__PURE__ */ jsxs("form", { onSubmit: handleSendMessage, className: "flex space-x-2", children: [
        /* @__PURE__ */ jsx(
          "input",
          {
            type: "text",
            value: newMessage,
            onChange: (e) => setNewMessage(e.target.value),
            placeholder: t.messagePlaceholder,
            className: "flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent",
            disabled: sending
          }
        ),
        /* @__PURE__ */ jsx(
          "button",
          {
            type: "submit",
            disabled: sending || newMessage.trim() === "",
            className: "px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors",
            children: sending ? t.sending : t.sendButton
          }
        )
      ] }),
      /* @__PURE__ */ jsx("div", { className: "mt-2 text-xs text-gray-500", children: t.responsePrompt })
    ] })
  ] });
}

export { MessageThread as M };
