...
const MOCK_ORDERS = [
  // 已报价 - 等待支付（报价阶段最后一步）
  {
    ...
    priceBreakdown: {
      ...
      materialCost: 45.2,
      // 材料费 (6061-T6铝合金)
      manufacturingService: 129.3,
      // 加工与品控服务
      shipping: 8,
      // 国际运费
      unitProductionCost: 174.5
      // 单件生产成本 (材料+加工)
    }
  },
  // 专家审核中 - 报价阶段
  ...
  // 制造中订单 - 订单阶段（已支付后）
  {
    ...
    // 价格明细breakdown
    priceBreakdown: {
      engineeringSetup: 260,
      // 一次性工程与设置费
      materialCost: 442.5,
      // 材料费 (钛合金)
      manufacturingService: 1897.49,
      // 加工与品控服务
      shipping: 35,
      // 国际运费
      unitProductionCost: 2339.99
      // 单件生产成本 (材料+加工)
    }
  },
  // 质量检测中订单
  ...
  // 支付确认订单 - 刚支付完成
  {
    ...
    priceBreakdown: {
      ...
      materialCost: 148.75,
      // 材料费 (6061铝合金)
      manufacturingService: 436.24,
      // 加工与品控服务
      shipping: 12,
      // 国际运费
      unitProductionCost: 584.99
      // 单件生产成本 (材料+加工)
    }
  },
  // 打包中订单
  ...
  // 另一个邮箱的测试数据 - 报价类型
  {
    ...
    priceBreakdown: {
      ...
      materialCost: 180,
      // 材料费 (黄铜)
      manufacturingService: 629.99,
      // 加工与品控服务
      shipping: 15,
      // 国际运费
      unitProductionCost: 809.99
      // 单件生产成本 (材料+加工)
    }
  }
];
function searchOrders(query) {
  ...
  return MOCK_ORDERS.filter((order) => {
    return order.email.toLowerCase().includes(searchQuery) || order.id.toLowerCase().includes(searchQuery) || order.shortId.toLowerCase() === searchQuery.toUpperCase() || searchQuery.includes(order.shortId.toLowerCase());
  });
}
function getMaterialLabel(material, language) {
  const materials = {
    zh: {
...
const POST = async ({ request }) => {
  ...
  try {
    ...
    const response = {
      ...
      message: localizedOrders.length > 0 ? language === "zh" ? `找到 ${localizedOrders.length} 个订单` : `Found ${localizedOrders.length} order(s)` : language === "zh" ? "未找到匹配的订单" : "No matching orders found",
      requestId: `track_${Date.now()}`,
      meta: {
        processingTime,
